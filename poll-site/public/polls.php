...
<?php else: ?>
    <?php foreach ($polls as $poll): ?>
        <div class="poll-card">
            ...
            <?php if ($has_voted): ?>
                <p style="color: green; margin-bottom: 10px;">&#10003; Вы уже проголосовали в этом опросе.</p>
                <a href="poll_view.php?id=<?= $poll['id'] ?>" class="action-btn btn-secondary">
                    Посмотреть результаты
                </a>
            <?php else: ?>
                <a href="poll_view.php?id=<?= $poll['id'] ?>" class="action-btn btn-primary">
                    Проголосовать
                </a>
            <?php endif; ?>
        </div>
