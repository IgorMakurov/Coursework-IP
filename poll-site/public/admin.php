...
if (isset($_GET['action']) && ($_GET['action'] == 'block' || $_GET['action'] == 'unblock') && isset($_GET['user_id'])) {
    ...
    try {
        ...
        if ($stmt->rowCount()) {
            $message = "Пользователь c ID {$user_id_to_toggle} теперь " . ($new_state ? "МОЖЕТ" : "НЕ МОЖЕТ") . " создавать опросы.";
        } else {
        ...
        <?php foreach ($users as $user): ?>
            <tr>
                ...
                <td style="border: 1px solid #ccc; padding: 8px;">
                    <?php if ($user['can_create_poll'] == 1): ?>
                        <a href="?action=block&user_id=<?= $user['id'] ?>" 
                        class="action-btn btn-danger-action">
                            Блокировать
                        </a>
                    <?php else: ?>
                        <a href="?action=unblock&user_id=<?= $user['id'] ?>" 
                        class="action-btn btn-success-action">
                            Разблокировать
                        </a>
                    <?php endif; ?>
                </td>
